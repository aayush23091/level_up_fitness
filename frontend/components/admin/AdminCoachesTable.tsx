        ...
        <div className="flex items-center gap-4 w-full sm:w-auto justify-between sm:justify-end">
          {totalCoaches > 0 && (
            <span className="text-xs text-muted font-bold uppercase tracking-wider">
              Total Coaches: <span className="text-accent">{totalCoaches}</span>
            </span>
          )}
        </div>
      </div>
            ...
            <tbody className="divide-y divide-border/60 text-sm">
              {loading ? (
                ...
                coaches.map((coach) => {
                  ...
                  return (
                    <tr key={coachId} className="hover:bg-card-secondary/20 transition-colors group">
                      <td className="px-6 py-4">
                          ...
                          Array.isArray(specialization) ? (
                            <div className="flex flex-wrap gap-1">
                              {specialization.slice(0, 2).map((spec, idx) => (
                                <span key={idx} className="px-2 py-0.5 bg-card-secondary text-foreground text-[10px] rounded-full">
                                  {spec}
                                </span>
                              ))}
                              {specialization.length > 2 && (
                                <span className="px-2 py-0.5 bg-card-secondary text-foreground text-[10px] rounded-full">
                                  +{specialization.length - 2}
                                </span>
                              )}
                            </div>
                      ...
                      <td className="px-6 py-4">
                        {experience ? (
                          <span className="text-foreground text-xs">{experience} years</span>
                        ) : (
                          <span className="text-muted text-xs">N/A</span>
                        )}
                      </td>
                      <td className="px-6 py-4">
                        {hireCost ? (
                          <span className="text-foreground text-xs font-medium">${hireCost}/hr</span>
                        ) : (
                          <span className="text-muted text-xs">N/A</span>
                        )}
                      </td>
                      <td className="px-6 py-4">
                        <span className="text-foreground text-xs font-medium">{clientCount}</span>
                      </td>
                      <td className="px-6 py-4">
                        <span className="text-foreground text-xs font-medium">{planCount}</span>
                      </td>
                      <td className="px-6 py-4 text-right">
            ...
            <div className="p-6 space-y-4">
              <div className="flex items-center gap-4">
                {selectedCoach.profilePhoto ? (
                  ...
                  <div className="w-16 h-16 rounded-full bg-card-secondary text-muted border border-border/30 flex items-center justify-center text-lg font-bold font-mono tracking-wider">
                    {getInitials(selectedCoach.name)}
                  </div>
                )}
                <div>
                  <p className="font-bold text-foreground text-lg">{selectedCoach.name}</p>
                  <p className="text-muted text-xs">{selectedCoach.email}</p>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-4 text-xs">
                <div className="space-y-1">
                  <p className="text-muted uppercase tracking-wider">Username</p>
                  <p className="text-foreground">{selectedCoach.username}</p>
                </div>
                <div className="space-y-1">
                  <p className="text-muted uppercase tracking-wider">Phone</p>
                  <p className="text-foreground">{(selectedCoach as any).phoneNumber || "N/A"}</p>
                </div>
                <div className="space-y-1">
                  <p className="text-muted uppercase tracking-wider">Experience</p>
                ...
                <div className="space-y-1">
                  <p className="text-muted uppercase tracking-wider">Hire Cost</p>
                  <p className="text-foreground">
                ...
                <div className="space-y-1 col-span-2">
                  <p className="text-muted uppercase tracking-wider">Specialization</p>
                  <div className="flex flex-wrap gap-1">
                    {(() => {
                ...
                <div className="space-y-1 col-span-2">
                  <p className="text-muted uppercase tracking-wider">Bio</p>
                  <p className="text-foreground">
                    {(selectedCoach.coachProfile?.bio || selectedCoach.bio) || "N/A"}
                  </p>
                </div>
                <div className="space-y-1">
                  <p className="text-muted uppercase tracking-wider">Clients</p>
                  <p className="text-foreground">{(selectedCoach as any).clientCount || 0}</p>
                </div>
                <div className="space-y-1">
                  <p className="text-muted uppercase tracking-wider">Workout Plans</p>
                  <p className="text-foreground">{(selectedCoach as any).planCount || 0}</p>
                </div>
                <div className="space-y-1 col-span-2">
                  <p className="text-muted uppercase tracking-wider">Joined</p>
                  <p className="text-foreground">{formatDate((selectedCoach as any).createdAt)}</p>
                </div>
              </div>
            </div>
